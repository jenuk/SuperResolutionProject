...
\begin{tikzpicture}[node distance=0.6cm]
	...
	\tikzstyle{transconv} = [layer, fill=lime!40]
	...
	\tikzstyle{upBlock} = [layer, fill=lime!60!green!40]
	...
	($(tr1) + (-0.9, -2.5)$) -- ($(tr3) + (0.5, -2.5)$) node [black,midway,below=0.3cm] {$\log_2(s)$};
	
	\node [draw, circle, rotate=90, below of=tr3, fill=white, node distance=1.6cm] (concat) {\rotatebox{-90}{$\doubleplus$}};
	
	\node [conv, below of=concat, node distance=1cm] (fromRGB1) {Conv: $1, 64$};
	...
	\begin{pgfonlayer}{box}
		\draw [fill=gray!10] ($(concat) - (0.7, 2.5)$) -| ($(fromRGB3) + (0.7, 2.5)$) -| node [pos=0.27, above] {Color Reconstruction} cycle ;
	\end{pgfonlayer}
	
	\node [layer, node distance=2cm, below of=fromRGB3] (output) {$I^{\mathrm{out}}$};
	
	\draw [data] ($(input) + (.6, 0)$) -- ++(0, -4.5) -| (concat)
	node [pos=0.3, fill=OliveGreen!60, rectangle, draw=black, thin, text=black, minimum height=1cm] {Nearest Neighbor Upsampling};
